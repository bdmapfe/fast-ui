<template>
    <label :class="['checkout-container','checkout-size-'+size]">
        <input @change="changeEvt"
               :class="{'checkout-choice': isChecked, 'checkbox-ani': animation}"
               type="checkbox" :name="name" @click="clickHandler" />
...
export default {
    ...
    methods: {
        clickHandler() {
            // 更新是否选中状态
            this.$emit('isCheckedChanged', !this.isChecked);
        },
        changeEvt() {
            this.$emit('checkboxValueChanged');
        }
    }
}
</script>
