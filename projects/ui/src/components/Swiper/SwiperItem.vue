<template>
    <transition :name="pageTransition" @enter="enter" @after-enter="afterEnter">
        <div v-show="active" class="swiper-item" :class="{ active }">
            <div ref="swiper-content" class="swiper-item-content"
                @mousedown="swipeStart" @mouseup="swipeMove" @touchstart="swipeStart" @touchmove="swipeMove">
                <slot />
            </div>
        </div>
    </transition>
...
export default {
    ...
    methods: {
        ...
        reachBoundary(direction) {
            const { x, y, maxScrollX, maxScrollY } = this.scroll;
            if (direction === 'Up') {
                return y === maxScrollY;
            }
            if (direction === 'Down') {
